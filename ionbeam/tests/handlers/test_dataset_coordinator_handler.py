...
def coordinator_handler(
    ...
    mock_ordered_queue,
    mock_coordinator_metrics: CoordinatorMetricsProtocol,
):
...
def _create_data_available_event(
    ...
    return DataAvailableEvent(
        id=event_id, metadata=metadata, start_time=start_time, end_time=end_time
    )
...
class TestDatasetCoordinatorHandler:
    """Test suite for DatasetCoordinatorHandler - focuses on coordinator responsibilities."""
    ...
    async def test_coordinator_stores_ingestion_record(
        ...
        await coordinator_handler._handle(event)

        # Event persisted
        records = await mock_ingestion_record_store.get_ingestion_records(
    ...
    async def test_coordinator_aggregates_multiple_events_into_single_window(
        ...
        metadata = _create_metadata(
            aggregation_span=timedelta(days=1), stc_window=timedelta(hours=1)
        )
        handler = DatasetCoordinatorHandler(
            DatasetCoordinatorConfig(),
            mock_ingestion_record_store,
            mock_ordered_queue,
            mock_coordinator_metrics,
    ...
    async def test_coordinator_splits_large_event_into_multiple_windows(
        ...
        expected_starts = [
            datetime(2024, 1, 1, h, 0, 0, tzinfo=timezone.utc) for h in (10, 11, 12)
        ]
    ...
    async def test_coordinator_detects_gaps_and_skips_incomplete_window(
        ...
        # 10:00-10:30
        e1 = _create_data_available_event(
            uuid4(), metadata, base, base + timedelta(minutes=30)
        )
        # 10:45-11:00 (gap 10:30-10:45)
        e2 = _create_data_available_event(
            uuid4(), metadata, base + timedelta(minutes=45), base + timedelta(hours=1)
        )
        # 11:00-12:00 full coverage next window
        e3 = _create_data_available_event(
            uuid4(), metadata, base + timedelta(hours=1), base + timedelta(hours=2)
        )
        ...
        assert len(queue) == 1
        wid = _window_id(
            datetime(2024, 1, 1, 11, 0, 0, tzinfo=timezone.utc), timedelta(hours=1)
        )
        assert f"{metadata.dataset.name}:{wid}" in queue
        ...
        assert (
            f"{metadata.dataset.name}:{wid_incomplete}" not in queue
        )  # But not enqueued
    ...
    async def test_coordinator_respects_stc_window(
        ...
        metadata = _create_metadata(
            aggregation_span=timedelta(hours=1), stc_window=timedelta(hours=2)
        )
        handler = DatasetCoordinatorHandler(
            DatasetCoordinatorConfig(),
            mock_ingestion_record_store,
    ...
    async def test_coordinator_skips_enqueue_when_observed_matches_desired(
        ...
        desired_hash = (
            __import__("hashlib")
            .sha256(",".join(sorted(desired_ids)).encode("utf-8"))
            .hexdigest()
        )
    ...
    async def test_coordinator_only_processes_spanned_windows(
        ...
        handler = DatasetCoordinatorHandler(
            DatasetCoordinatorConfig(only_process_spanned_windows=True),
            mock_ingestion_record_store,
            mock_ordered_queue,
            mock_coordinator_metrics,
        )
        ...
        rec1 = IngestionRecord(
            id=uuid4(),
            metadata=metadata,
            start_time=early_start,
            end_time=early_start + timedelta(minutes=30),
        )
        rec2 = IngestionRecord(
            id=uuid4(),
            metadata=metadata,
            start_time=early_start + timedelta(minutes=30),
        ...
        await handler._handle(current)

        queue = mock_ordered_queue.get_queue_dict()
        assert len(queue) == 1
        wid_current = _window_id(
            datetime(2024, 1, 1, 10, 0, 0, tzinfo=timezone.utc), timedelta(hours=1)
        )
        assert f"{metadata.dataset.name}:{wid_current}" in queue
        wid_early = _window_id(
            datetime(2024, 1, 1, 8, 0, 0, tzinfo=timezone.utc), timedelta(hours=1)
        )
    ...
    async def test_coordinator_processes_all_windows_when_flag_disabled(
        ...
        handler = DatasetCoordinatorHandler(
            DatasetCoordinatorConfig(only_process_spanned_windows=False),
            mock_ingestion_record_store,
            mock_ordered_queue,
            mock_coordinator_metrics,
        )
    ...
    async def test_coordinator_handles_backfill_completing_window(
        ...
        metadata = _create_metadata(
            aggregation_span=timedelta(hours=1), stc_window=timedelta()
        )
        handler = DatasetCoordinatorHandler(
            DatasetCoordinatorConfig(),
            mock_ingestion_record_store,
        ...
        await handler._handle(event_a_full)
    ...
    async def test_coordinator_re_enqueues_when_new_event_changes_desired_state(
        ...
        observed_hash = (
            __import__("hashlib")
            .sha256(",".join(sorted(ids)).encode("utf-8"))
            .hexdigest()
        )
    ...
    async def test_coordinator_allows_incomplete_windows_when_configured(
        ...
        """Coordinator: Windows with incomplete coverage are enqueued when allow_incomplete_windows is True."""
        metadata = _create_metadata(
            aggregation_span=timedelta(hours=1), stc_window=timedelta(hours=1)
        )
        handler = DatasetCoordinatorHandler(
    ...
    async def test_coordinator_allows_windows_with_gaps_when_configured(
        ...
        """Coordinator: Windows with gaps are enqueued when allow_incomplete_windows is True."""
        metadata = _create_metadata(
            aggregation_span=timedelta(hours=1), stc_window=timedelta(hours=1)
        )
        handler = DatasetCoordinatorHandler(
