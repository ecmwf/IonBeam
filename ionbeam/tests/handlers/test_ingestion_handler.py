...
from ionbeam.handlers.ingestion_handler import IngestionConfig, IngestionHandler
...
def ingestion_handler(
    mock_timeseries_db,
    mock_ingestion_metrics: IngestionMetricsProtocol,
    mock_arrow_store,
):
    config = IngestionConfig(batch_size=1000)
    return IngestionHandler(
        config, mock_timeseries_db, mock_ingestion_metrics, mock_arrow_store
    )
...
class TestIngestionHandler:
    ...
    async def test_ingestion_writes_data_to_timeseries(
        self, ingestion_handler, sample_metadata, sample_payload_key, mock_timeseries_db
    ):
        ...
        result = await ingestion_handler._handle(command)
        ...
        expected_columns = {
            ...
            LatitudeColumn,
            LongitudeColumn,
            "air_temperature__deg_C__0.0__point__PT0S",
            "station_id",
        }
