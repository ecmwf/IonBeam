...
async def test_failed_build_reschedules_the_window(
    ...
    window = await _seed_window(
        mock_ingestion_record_store, datetime(2024, 1, 1, 10, tzinfo=timezone.utc)
    )
    published = []
    builder = _builder(
        mock_ingestion_record_store,
    ...
    assert mock_arrow_store.stored_keys() == []
    assert published == []
...
async def test_worker_loop_drains_the_queue_with_concurrent_builds(
    ...
    async with builder:
        ...
        while len(mock_arrow_store.stored_keys()) < len(windows):
            assert asyncio.get_event_loop().time() < deadline, "builds did not finish"
            await asyncio.sleep(0.05)

    assert len(mock_arrow_store.stored_keys()) == len(windows)
...
class TestRecordScopedBuilds:
    ...
    async def test_build_composes_exactly_the_desired_records(
        ...
        desired = await _seed_record(
            mock_ingestion_record_store, arrived_at=_arrived(1)
        )
    ...
    async def test_unreachable_record_rows_defer_the_build(
        ...
        await _seed_record(mock_ingestion_record_store, arrived_at=_arrived(2))
...
class TestWindowManifests:
    ...
    async def test_manifest_names_the_records_and_schema_of_the_build(
        ...
        assert build.source == "record_set"
    ...
    async def test_rebuilds_append_to_the_build_history(
        ...
        db = InMemoryTimeSeriesDatabase()
        first = await _seed_record(mock_ingestion_record_store, arrived_at=_arrived(1))
        ...
        second = await _seed_record(
            mock_ingestion_record_store, arrived_at=_arrived(30)
        )
        ...
        assert mock_arrow_store.stored_keys() == sorted([v1, v2])
        assert mock_arrow_store.get_total_rows(v1) == 1
        assert mock_arrow_store.get_total_rows(v2) == 2
...
async def test_worker_loop_survives_a_queue_outage(
    ...
    async with builder:
        deadline = asyncio.get_event_loop().time() + 10.0
        while not mock_arrow_store.stored_keys():
...
async def test_build_file_footer_carries_its_own_provenance(
    ...
    db = InMemoryTimeSeriesDatabase()
    record = await _seed_record(mock_ingestion_record_store, arrived_at=_arrived(1))
    ...
    schema = pq.read_schema(
        mock_arrow_store._get_path(published[0].dataset_locations[0])
    )
