...
async def _seed_window(store, db, start: datetime) -> Window:
    """One fully covered window: its desired record set, and rows to build from."""
    ...
    await _seed_rows(db, str(record.id), [0, 30], [20.0, 21.0], start=start)
    return window


class FailingTimeSeriesDatabase(TimeSeriesDatabase):
    """Timeseries adapter whose every call fails, for boundary-outage flows."""

    async def query_measurement_data(self, **kwargs):
...
@pytest.fixture
def make_builder(coordination_store, build_queue, arrow_store, builder_metrics, published):
    """A builder over the shared in-memory backends, publishing into ``published``."""

    def _make(timeseries_db, **config) -> DatasetBuilder:
...
@pytest.fixture
def build_artifact(make_builder, arrow_store, published):
    """Build WINDOW and return the artifact the builder published."""

    async def _build(timeseries_db, **config) -> pa.Table:
        await make_builder(timeseries_db, **config).build_window(WINDOW)
        assert len(published) == 1
        return pa.concat_tables(
            arrow_store.table(location)
...
@pytest.fixture
def build_manifest(make_builder, arrow_store):
    """Build WINDOW and return the manifest stored beside its artifact."""

    async def _build(timeseries_db, **config) -> WindowManifest:
        await make_builder(timeseries_db, **config).build_window(WINDOW)
...
async def test_failed_build_reschedules_the_window(
    ...
    window = await _seed_window(
        coordination_store,
        InMemoryTimeSeriesDatabase(),
        datetime(2024, 1, 1, 10, tzinfo=timezone.utc),
    )
    builder = make_builder(failing_timeseries_db)
...
async def test_worker_loop_drains_the_queue_with_concurrent_builds(
    ...
    builder = make_builder(timeseries_db, concurrency=2)
...
async def _seed_rows(
    ...
    n = len(minutes)
    origin = start if start is not None else WINDOW_START
    table = pa.table(
        {
            "time": pa.array(
                [origin + timedelta(minutes=m) for m in minutes],
...
class _QueryCountingDatabase(InMemoryTimeSeriesDatabase):
    """Records the time span of every read, so a test can pin how a build
    divides its window."""

    def __init__(self) -> None:
        super().__init__()
        self.spans_queried: list[tuple[datetime, datetime]] = []

    def query_measurement_data(self, **kwargs):
        self.spans_queried.append((kwargs["start_time"], kwargs["end_time"]))
        return super().query_measurement_data(**kwargs)
...
async def test_a_chunked_read_builds_what_a_single_read_builds(
    coordination_store,
    build_artifact,
):
    """The artifact is identical and time-ascending however many chunks the
    read took."""
    db = _QueryCountingDatabase()
    record = await _seed_record(coordination_store, arrived_at=_arrived(1))
    minutes = [0, 4, 5, 9, 10, 17, 29, 30, 44, 59]
    await _seed_rows(db, record, minutes, [20.0 + m for m in minutes])

    chunked = await build_artifact(db, chunk_span=timedelta(minutes=5))

    times = chunked.column("time").to_pylist()
    assert times == sorted(times)
    assert chunked.column("temperature").to_pylist() == [20.0 + m for m in minutes]
    assert len(db.spans_queried) > 1


async def test_a_correction_wins_across_a_chunk_boundary(
    coordination_store,
    build_artifact,
):
    """An identity carries its timestamp, so a row's revisions always share
    a chunk. A correction seeded exactly on a chunk boundary still replaces
    the row it corrects."""
    db = InMemoryTimeSeriesDatabase()
    original = await _seed_record(
        coordination_store,
    ...
    await _seed_rows(db, original, [0, 5, 10], [20.0, 21.0, 22.0])
    await _seed_rows(db, correction, [5], [18.5])

    table = await build_artifact(db, chunk_span=timedelta(minutes=5))
    assert table.column("temperature").to_pylist() == [20.0, 18.5, 22.0]
...
async def test_unreachable_record_rows_defer_the_build(
    ...
    builder = make_builder(db, retry_backoff_base_seconds=0.01)
    await builder.build_window(WINDOW)
...
async def test_a_rebuild_appends_to_the_manifest_build_history(
    coordination_store,
    arrow_store,
    build_manifest,
):
    ...
    assert arrow_store.table(v1).num_rows == 1
    assert arrow_store.table(v2).num_rows == 2
...
async def test_repeated_failures_park_the_window_until_a_build_succeeds(
    ...
    builder = make_builder(db, max_build_attempts=2, retry_backoff_base_seconds=0.01)
...
async def test_a_deferring_window_frees_its_slot_for_other_builds(
    ...
    builder = make_builder(db, concurrency=1, retry_backoff_base_seconds=300.0)
    async with builder:
        deadline = asyncio.get_event_loop().time() + 10.0
        while not published:
...
async def test_worker_loop_survives_a_queue_outage(
    ...
    window = await _seed_window(
        coordination_store, timeseries_db, datetime(2024, 1, 1, 10, tzinfo=timezone.utc)
    )
    await build_queue.schedule(window, window.end)
    ...
    builder = make_builder(timeseries_db, retry_backoff_base_seconds=0.01)
...
async def test_build_file_footer_carries_its_own_provenance(
    ...
    builder = make_builder(db)
    await builder.build_window(WINDOW)

    schema = arrow_store.schema_of(published[0].dataset_locations[0])
