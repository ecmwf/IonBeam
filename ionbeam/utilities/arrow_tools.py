...
from ionbeam.models.models import DataIngestionMap
...
def _enforce_schema(df: pd.DataFrame, schema: pa.Schema) -> pd.DataFrame:
    """
    Ensure the DataFrame has all fields present in the provided schema and in the same order.
    Missing columns are filled with reasonable defaults based on Arrow type.
    """
    if schema is None:
        return df
    ...
    # Re-order and drop extra columns.
...
async def dataframes_to_tables(
    ...
    *,
    ...
    async for df in _iterate_dataframes(dataframes):
        ...
        table = pa.Table.from_pandas(chunk, schema=schema, preserve_index=preserve_index)
        yield table
        await asyncio.sleep(0)
...
async def dataframes_to_record_batches(
    ...
    *,
    ...
    """
    Convert a (possibly async) stream of pandas DataFrames into Arrow RecordBatches.

    Args:
        dataframes: Iterable or async iterable yielding DataFrames.
        schema: Optional Arrow schema to enforce on each chunk.
        preserve_index: Whether to keep pandas index columns in the Arrow output.
    """
