...
def coerce_types(df: pd.DataFrame, ingestion_map: DataIngestionMap, use_canonical_names = False):
    ...
    """
    src_time = 'datetime' if use_canonical_names or ingestion_map.datetime.from_col is None else ingestion_map.datetime.from_col
    src_lat = 'lat' if use_canonical_names or ingestion_map.lat.from_col is None else ingestion_map.lat.from_col
    src_lon = 'lon' if use_canonical_names or ingestion_map.lon.from_col is None else ingestion_map.lon.from_col
