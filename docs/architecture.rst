...
**Ionbeam Core**
  A service hosting event handlers that process observations:
...
3. **Dataset Building**: The builder handler processes queued windows by querying InfluxDB, generating Arrow RecordBatch streams in object storage, and publishing :ref:`messaging-interface:DataSetAvailableEvent` messages that fan out to all registered exporters.
...
.. list-table::
   ...
   * - ``ionbeam.data.available.aggregation``
     - DataAvailableEvent
     - Ionbeam Core (Coordinator Handler)
