...
from ecmwf import ODBExporter, ODBExporterConfig, VarNoMapping
...
async def sample_dataset_key(arrow_store_writer) -> str:
    ...
    mslp_col = (
        "air_pressure_at_mean_sea_level__Pa__1.0__mean__PT1M"  # maps to varno 108
    )
...
class TestODBExporter:
    """Test suite for ODBExporter."""

    @pytest.mark.asyncio
    async def test_exporter_creates_odb_with_correct_mapping(
        self,
        ...
        """Test that exporter creates ODB file with correct canonical column mapping."""
        ...
        # Get batch stream from arrow store
        batch_stream = mock_arrow_store.read_record_batches(sample_dataset_key)

        await odb_exporter.export_handler(event, batch_stream)
        ...
        for col in expected_columns:
            assert col in odb_df.columns, (
                f"Expected column {col} not found in ODB output"
            )
