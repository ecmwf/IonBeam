...
def construct_sck_authors(session, device):
    owner = device["owner"]
    return [session.query(db.Author).filter_by(name = owner["username"], external_id = owner["id"]).one_or_none() \
        or db.Author(
        name = owner["username"],
        external_id = owner["id"],),
        
        session.query(db.Author).filter_by(name = "I-CHANGE").one_or_none() or db.Author(
        name = "I-CHANGE",
...
def construct_sck_sensors(sck_source, session, sensors):
    ...
    for s in sensors:
        ...
        try:
            ...
            logger.warning(f"Could not find variable with key {(property_name, unit)} in the mappings of Smart Citizen Kit source")
            continue
