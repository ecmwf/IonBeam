...
def coerce_types(
    df: pd.DataFrame, ingestion_map: DataIngestionMap, use_canonical_names: bool = False
) -> pd.DataFrame:
    src_time = (
        ObservationTimestampColumn
        if use_canonical_names or ingestion_map.datetime.from_col is None
        else ingestion_map.datetime.from_col
    )
    src_lat = (
        LatitudeColumn
        if use_canonical_names or ingestion_map.lat.from_col is None
        else ingestion_map.lat.from_col
    )
    src_lon = (
        LongitudeColumn
        if use_canonical_names or ingestion_map.lon.from_col is None
        else ingestion_map.lon.from_col
    )
