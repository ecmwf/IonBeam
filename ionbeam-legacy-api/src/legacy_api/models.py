...
class MarsSelectionModel(BaseModel):
    ...
    aggregation_type: str = Field(
        default="by_time", description="Data aggregation type"
    )
    ...
    time: Optional[str] = Field(
        None,
        description="Time or time range in MARS format (HHMM or HHMM/to/HHMM/by/1).",
    )
...
class StationResponse(BaseModel):
    ...
    aggegation_type: str = Field(
        default="by_time", description="Aggregation type (note: legacy spelling)"
    )
    ...
    authors: List[AuthorModel] = Field(
        ..., description="List of data authors/platforms"
    )
    mars_selection: MarsSelectionModel = Field(
        ..., description="MARS selection parameters"
    )
...
class ListResult(BaseModel):
    ...
    datetime: dt = Field(..., description="The datetime of the data")
