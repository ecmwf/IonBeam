...
class LegacyAPIDataService:
    ...
    def query_stations(
        ...
        try:
            ...
            logger.debug(
                f"Executing station metadata query - query={query}, params={params}"
            )
            ...
            for idx, row in result.iterrows():
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse metadata row: {e}, row_dict={row_dict}"
                    )
                    continue
    ...
    def query_observations(
        ...
        try:
            ...
            data_pattern = str(
                self.data_dir / f"platform={platform}" / "**" / "*.parquet"
            )
            ...
            logger.debug(
                "Executing observation query - query=%s, params=%s", query, params
            )
