...
class IonbeamLegacyProjectionService:
    """
    ...
    METEOTRACKER_COLUMN_MAP = {
        ...
        "mole_fraction_of_carbon_monoxide_in_air__1e-6__0.0__point__PT0S": "carbon_monoxide",
        "ozone__1e-9__0.0__point__PT0S": "ozone",
        "mass_concentration_of_pm1_ambient_aerosol_in_air__ug m-3__0.0__point__PT0S": "particulate_matter_1",
        "mass_concentration_of_pm2p5_ambient_aerosol_in_air__ug m-3__0.0__point__PT0S": "particulate_matter_2_5",
        "mass_concentration_of_pm4_ambient_aerosol_in_air__ug m-3__0.0__point__PT0S": "particulate_matter_4",
        "particulate_matter_particle_number_0_5__cm-3__0.0__point__PT0S": "particulate_matter_particle_number_0_5",
        "particulate_matter_particle_number_1__cm-3__0.0__point__PT0S": "particulate_matter_particle_number_1",
        "particulate_matter_particle_number_2_5__cm-3__0.0__point__PT0S": "particulate_matter_particle_number_2_5",
        "particulate_matter_particle_number_4__cm-3__0.0__point__PT0S": "particulate_matter_particle_number_4",
        "mole_fraction_of_carbon_dioxide_in_air__1e-6__0.0__point__PT0S": "carbon_dioxide",
        "epa_air_quality__1__0.0__point__PT0S": "EPA_Air_Quality",
        "fast_air_quality__1__0.0__point__PT0S": "Fast_Air_Quality",
        "mole_fraction_of_sulphur_dioxide_in_air__1e-6__0.0__point__PT0S": "sulphur_dioxide",
        "mass_concentration_of_pm10_ambient_aerosol_in_air__ug m-3__0.0__point__PT0S": "particulate_matter_10",
        "particulate_matter_particle_number_10__cm-3__0.0__point__PT0S": "particulate_matter_particle_number_10",
    }
    ...
    # Column name constants - these should match the ionbeam core constants
    ...
    def __init__(
        ...
        self._platform_configs: dict[str, PlatformConfig] = {
            "meteotracker": PlatformConfig(
                ...
                station_name_builder=lambda df, p: "meteotracker: "
            ...
            "acronet": PlatformConfig(
                ...
                station_name_builder=lambda df, p: df.get(
                    "station_name", df["station_id"]
                ),
                extra_fields={},
            ),
        }

        self._station_name_resolver: dict[
            str, Callable[[str, pd.DataFrame, str], str]
        ] = {
            "meteotracker": lambda sid, grp, platform: f"{platform}: {sid}",
            "acronet": lambda sid, grp, platform: grp["station_name"].iloc[-1]
            if "station_name" in grp.columns
            else sid,
        }

    def _transform_raw_data(
        self, table: pa.Table, platform: str, config: PlatformConfig
    ) -> pa.Table:
        df = table.to_pandas()
        ...
        dt_series = df[self.OBSERVATION_TIMESTAMP_COLUMN]
        ...
        for field_name, extractor in config.extra_fields.items():
            legacy_df[field_name] = (
                extractor(df) if (result := extractor(df)) is not None else None
            )
    ...
    def _aggregate_station_metadata(self, table: pa.Table, platform: str) -> pa.Table:
        ...
        name_resolver = self._station_name_resolver.get(
            platform, lambda sid, grp, p: f"{p}: {sid}"
        )
        ...
        for station_id, group in df.groupby("station_id"):
            ...
            coords = group[[self.LONGITUDE_COLUMN, self.LATITUDE_COLUMN]].values
            lons = coords[:, 0]
            lats = coords[:, 1]
            ...
            results.append(
                {
                    ...
                    "description": None,
                    "aggregation_type": "by_time",
                    "time_span_start": pd.to_datetime(
                        group[self.OBSERVATION_TIMESTAMP_COLUMN].min(), utc=True
                    ),
                    "time_span_end": pd.to_datetime(
                        group[self.OBSERVATION_TIMESTAMP_COLUMN].max(), utc=True
                    ),
                    "location_lat": lat,
                    "location_lon": lon,
    ...
    def _merge_metadata(self, new_table: pa.Table, platform: str) -> None:
        ...
        if not metadata_file.exists():
            ...
            self._write_parquet_atomically(
                metadata_file, pa.Table.from_pandas(new_df, schema=self.METADATA_SCHEMA)
            )
            self.logger.info(
        ...
        if non_conflicting_existing:
            results.extend(
                existing_df[
                    existing_df["internal_id"].isin(non_conflicting_existing)
                ].to_dict("records")
            )
        if non_conflicting_new:
            results.extend(
                new_df[new_df["internal_id"].isin(non_conflicting_new)].to_dict(
                    "records"
                )
            )
        ...
        for internal_id in conflicting_ids:
            ...
            for wkt_str in pd.concat(
                [existing_records["geometry_wkt"], new_records["geometry_wkt"]]
            ).dropna():
            ...
            if all_bounds:
                minx, miny, maxx, maxy = (
                    func(col)
                    for func, col in zip((min, min, max, max), zip(*all_bounds))
                )
        ...
        self._write_parquet_atomically(
            metadata_file,
            pa.Table.from_pandas(pd.DataFrame(results), schema=self.METADATA_SCHEMA),
        )
        self.logger.info(
    ...
    def _write_parquet_atomically(
        self, file_path: pathlib.Path, table: pa.Table
    ) -> None:
    ...
    async def persist_batches(
        ...
        if not batches:
            self.logger.warning("No data to persist", platform=platform)
            return
        ...
        if (platform_config := self._platform_configs.get(platform)) is None:
            self.logger.warning(
                "No platform configuration found; skipping projection",
                platform=platform,
            )
            return
        ...
        if legacy_table.num_rows == 0:
            self.logger.warning("No rows after transformation", platform=platform)
            return
        ...
        for (observation_date, observation_hour), hour_group in df.groupby(
            ["_observation_date", "_observation_hour"]
        ):
            partition_path = (
                raw_dir
                / f"platform={platform}"
            ...
            hour_group_clean = hour_group.drop(
                columns=[
            ...
            hour_table = pa.Table.from_pandas(
                hour_group_clean, schema=legacy_table.schema
            )
