...
class CIMA_API():
    ...
    def __init__(self,
        ...
        self.oauth.fetch_token(token_url=self.endpoints.token_endpoint,
    ...
    def list_stations_by_sensor(self, name, stationgroup="ComuneLive%IChange", geowin = "6,36,18.6,47.5"):
        ...
        if name not in self.sensor_names.IT:
            raise ValueError(f"sensor name '{name}' is not in the list of allowed sensor types, \
                            neither English nor Italian, closest match {self.match_sensor_names(name)}")
        
        r = self.get(self.api_url + f"sensors/list/{name}", 
        ...
        for sensor_name in self.sensor_names.IT:
            ...
            for sensor_info in stations:
                ...
                if station_id not in all_stations: 
                    all_stations[station_id] = dict(
                            name = station_name,
                            lat = sensor_info['lat'], 
                            lon = sensor_info['lng'], 
                            sensors = {})
                    
                station_info = all_stations[station_id]
                ...
                sensor_record = {
                                "unit" : sensor_info['mu'], 
                                "id" : sensor_info['id'], # This an alphanumeric ID provided by the API for all streams of readings, i.e all unique (station_id, sensor_type) pairs
                                }

                station_info['sensors'][sensor_name] = sensor_record
                
                if sensor_name not in all_sensors:
                    all_sensors[sensor_name] = dict(unit = sensor_info['mu'], translation = self.sensor_name_translations_IT2EN[sensor_name])
